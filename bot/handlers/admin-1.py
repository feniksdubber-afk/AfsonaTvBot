...
# ── KINO QO'SHISH — add_movie.py ga yo'naltirilgan ───────────────────
@router.message(F.text == "🎬 Kino qo'shish", F.from_user.id.in_(ADMINS))
async def start_add_content(message: Message):
    await message.answer(
        "🎬 <b>Kino/Serial qo'shish</b>\n\n"
        "Video yuboring + caption (tagida matn) yozing:\n\n"
        "<code>O'zbek nomi\n"
        "Русское название\n"
        "2024\n"
        "Janr\n"
        "Mamlakat\n"
        "Dublyaj tili</code>\n\n"
        "Serial uchun qo'shing: <code>s1e1</code>\n"
        "Premium uchun: <code>premium</code>",
        parse_mode="HTML"
    )
